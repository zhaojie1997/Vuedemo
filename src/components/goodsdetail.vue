...
<script>
	...
	import order from '../api/getorder'
		...
		methods: {
			...
			async get() {
				if (localStorage.getItem("token") != null) {
                    //自定义代码
					let str="&token="+window.localStorage.getItem('token')
					window.console.log(str)
                    this.data = await getdata.fun(this.pid+""+str);
                }
                if (localStorage.getItem("token") == null) {
                    //自定义代码
			...
			async send() {if(this.prompt=='是否要加入购物车?'){
                    window.console.log(window.localStorage.getItem('token'), this.lessonId)
                    this.sure = false;
                    this.addflag = await addtocart.fun({
                        "token":window.localStorage.getItem('token'),
                        "lessonId": this.lessonId
                    });
				}else{window.console.log(this.prompt)

                this.order = await order.fun({
                    'pid':this.lessonId,
                    'token':window.localStorage.getItem('token')

                })
                window.console.log(this.order)
                this.$router.push({ path:'/zhifu?'+this.pid+'&cost='+this.data.lessonData.lessonPrice+'&from=gd&' +this.lessonId})
                this.sure = false;
            }
...
this.$router.go(-1)
		...
		beforeMount() {
			...
			this.lessonId=this.$route.query.pid
